...
const createUser = async function (req, res) {
    try {
        ...
        if (!isValid(data.address.city)) return res.status(400).send({ status: false, msg: "The Password Attributes should not be empty" })
        if (!isValidName(data.address.city)) return res.status(400).send({ status: false, msg: "Pls Enter Valid city Name" })

        if (!isValid(data.address.pincode)) return res.status(400).send({ status: false, msg: "The Password Attributes should not be empty" })
